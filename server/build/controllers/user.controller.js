...
//update access token  check env pls
exports.updateAccessToken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!session) {
            return next(new ErrorHandler_1.default("Login To Access", 400));
        }
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); //7 day expiry
        return next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!oldPassword || !newPassword) {
            return next(new ErrorHandler_1.default("Password cannot be empty!", 400));
        }
        const user = await userModel_1.default.findById(req.user?._id).select("+password");
        if (user?.password === undefined) {
            return next(new ErrorHandler_1.default("Social Auth || Continue with email", 404));
        }
        const isPasswordMatch = await user?.comparePassword(oldPassword);
        if (!isPasswordMatch) {
            return next(new ErrorHandler_1.default("Password Doesn't match", 400));
        }
...
exports.updateUserRole = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const isUserExist = await userModel_1.default.findOne({ email });
        if (isUserExist) {
...
exports.deleteUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!user) {
            return next(new ErrorHandler_1.default("User Not found || Invalid user id", 404));
        }
